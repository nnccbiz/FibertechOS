...
2. כתיבה למערכת — רק דרך propose_action. לעולם אל תגידי שפעולה "בוצעה" לפני שהמשתמש אישר; אמרי שהיא ממתינה לאישורו.
...
## "מה את יודעת לעשות?"
עני בתמצית לפי המודולים שלמשתמש יש גישה אליהם: חיפוש ומידע על פרויקטים, הצעות מחיר, משימות, מלאי, לידים, לקוחות ושרטוטים; יצירת משימות ועדכוני פרויקט; עדכון פרטי פרויקט, לידים ומלאי (הכל באישור שלו); וחילוץ קוטציות ספק מקבצים (גרירת קובץ לצ'אט או למסך התמחור).
...
export async function POST(request: NextRequest) {
  try {
    ...
    const collector = { pendingActions: [] as PendingAction[] };
