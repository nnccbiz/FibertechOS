...
export default function PendingRequestsList({ initial }: { initial: Request[] }) {
  ...
  if (!requests.length) {
    return (
      <div className="bg-white rounded-xl border border-line-subtle p-10 text-center text-content-muted">
  ...
  return (
    <div className="space-y-3">
      ...
      {requests.map((r) => {
        ...
        return (
          <div key={r.id} className="bg-white rounded-xl border border-line-subtle overflow-hidden">
            <button
...
function ApprovalForm({
  ...
  return (
    <div className="border-t border-line-subtle bg-neutral-50 p-4 space-y-4">
      <div>
        <div className="text-sm font-medium text-content-body mb-2">תבנית מהירה</div>
        <div className="flex gap-2 mb-4">
          <button onClick={() => applyPreset('admin')}  className="text-[13px] bg-white border border-line-strong px-3 py-1.5 rounded-lg hover:bg-neutral-100">Admin</button>
          <button onClick={() => applyPreset('member')} className="text-[13px] bg-white border border-line-strong px-3 py-1.5 rounded-lg hover:bg-neutral-100">Member (ברירת מחדל)</button>
          <button onClick={() => applyPreset('viewer')} className="text-[13px] bg-white border border-line-strong px-3 py-1.5 rounded-lg hover:bg-neutral-100">Viewer</button>
        </div>
      </div>

      <div>
        <div className="text-sm font-medium text-content-body mb-2">הרשאה לכל מודול</div>
      ...
      <div>
        <label className="block text-sm font-medium text-content-body mb-1">הערות (אופציונלי)</label>
        <input
          type="text"
          value={notes}
          onChange={(e) => setNotes(e.target.value)}
          className="w-full border border-line-subtle rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-primary-100"
        />
      </div>
      ...
      <div className="flex flex-wrap gap-2 pt-2 border-t border-line-subtle">
        ...
        <button
          ...
          disabled={pending}
          className="bg-neutral-200 text-content-body font-medium px-4 py-2 rounded-lg hover:bg-neutral-300 disabled:opacity-50"
        ...
        <button
          ...
          disabled={pending}
          className="bg-danger-soft text-danger font-medium px-4 py-2 rounded-lg hover:bg-danger-soft disabled:opacity-50"
