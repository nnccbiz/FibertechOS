...
const STATUS: Record<string, { label: string; color: string }> = {
  ...
  expired: { label: 'פג תוקף', color: 'bg-warning-soft text-warning' },
};
  ...
  return (
    <div className="bg-white rounded-xl border border-line-subtle p-5" dir="rtl">
      ...
      {loading ? (
        ...
        <p className="text-neutral-400 text-center py-4">אין הצעות פתוחות</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-[13px]">
            <thead>
              <tr className="text-neutral-400 text-[11px] text-right border-b border-line-subtle">
                ...
                return (
                  <tr key={q.id} className="border-b border-line-subtle hover:bg-neutral-50">
                    ...
                    <td className="py-2 text-left whitespace-nowrap">
                      {q.project_id && (
                        <button onClick={() => router.push(`/projects/${q.project_id}/quote/${q.id}`)} className="text-[12px] text-primary hover:underline">פתח ↗</button>
                      )}
                    </td>
                  </tr>
                );
