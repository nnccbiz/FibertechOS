...
- **`components/procurement/PODocument.tsx`** — branded A4 PO document (same visual language + html2canvas/jsPDF export pattern as `QuoteDocument`, simpler estimate pagination). Bilingual header (הזמנת רכש / Purchase Order), supplier addressee, LTR item descriptions, totals in the PO currency, signature block.
