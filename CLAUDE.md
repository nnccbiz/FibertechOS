...
- **Unit length & units count (2026-07-26)**: `quote_items.length_m` drives a computed "יחידות" column (qty ÷ length) in the editor, the read-only table and the quote document (headers carry English sublabels: DN / m / pipe # / Qty / ₪; length always shows one decimal). Editing units back-calculates qty (commit on blur/Enter). Defaults fill only an EMPTY length_m: regular pipe → 5.7, rocker → 2×DN (`defaultLengthM` in usePricing). Fractional units render red, and `updateQuoteStatus` BLOCKS issuing (status='sent') while any line has fractional units. The same length/units columns exist in the cost-input editor+display (`cost_input_items.length_m`) and on PO lines (`import_order_items.length_m`, migration `20260726_001` — copied from cost items on PO creation, shown in the PO editor and the supplier PDF as Unit L (m) / Units (pipe #)).
- **Full cost chain**: Supplier foreign price -> exchange rate -> ILS cost -> overheads -> profit -> selling price.
- **Item types**: `pipe_with_coupling`, `pipe_bare`, `coupling`, `wall_coupling`, `roker`, `floating_roker`, `buoy`, `elbow`, `flange`, `reducer`, `spacer` (חיוץ, added 2026-09-23), `other`. The picker is a comma-separated multi-select.
- **Length-based vs piece-counted (2026-09-23)**: `isLengthBased(item_type, product_name)` in `lib/pricing.ts` — only `pipe_with_coupling`/`pipe_bare`/`roker` have a meaningful "יחידות" count (qty ÷ length_m). A piece-counted part (elbow, חיוץ, מצוף, נזיר צף, flange…) may well carry a length, but each is one unit, so the units cell shows "—", the units input is disabled, and the fractional-units **issue gate in `updateQuoteStatus` skips it**. Applied in all 9 sites: both cost-item editors + display, the quote editor + read-only table, QuoteDocument, PODocument and the /procurement PO editor. Rows with no `item_type` (legacy, and PO lines which have no such column) fall back to the product-name keywords `defaultLengthM` uses, so nothing that worked before changes.
...
- **Quote-level discounts (2026-09-15)**: `quotes.global_discount_pct` (%) + `quotes.discount_amount` (migration `20260915_001`, denominated in the quote's own currency) — the fixed discount is applied AFTER the % discount: subtotal → pct → amount → VAT. Both inputs sit on the quote card row (edit mode only); summary lines render in QuoteItemsDisplay + QuoteDocument (PDF/public) only when > 0. `duplicateQuote` carries both.
- **Foreign-currency quotes (2026-09-23, edge case)**: `quotes.currency` (existing, default `'ILS'`) + `quotes.fx_rate`/`fx_rate_date` (migration `20260923_001`). **All internal pricing stays ILS** — cost chain, margins, `total_amount`, collections, production orders and reports are untouched. Picking USD/EUR in the draft quote card's מטבע ההצעה selector calls `setQuoteCurrency` → fetches the BoI rate and **locks it on the quote** (refreshable while draft via `refreshQuoteFxRate`; `updateQuoteStatus` blocks issuing a foreign quote with no rate). The **editor and the read-only table also switch** to that currency for the selling columns (מחיר מכירה / סה״כ + the totals row) while cost/overheads stay ILS: the editor's unit-price field is edited in the quote currency and committed back to ILS on blur/Enter (`defaultValue` + `key` remount — the FileList/units-input pattern; typing live would fight decimals), so a converted price can be rounded to a clean figure. `QuoteDocument` presents everything in that currency: printed unit price = ILS ÷ rate rounded to 2 decimals and the line total is computed FROM the printed price (same rule as whole-shekel ILS), column headers carry ($)/(€), **the VAT line is dropped entirely** (export sale — not shown as 0%), and the peg note is replaced by a rate note. The read-only table carries an azure "שווי בשקלים" footer row for the internal read. `quotes.currency` never relabels `total_amount` (import/procurement lists and the signed-quote webhook force ₪).
...
- **Stage gate**: `import_orders.po_sent_at` (+`po_sent_by`, migration `20260719_003`). NULL → the PO is "בהכנה" and lives ONLY in /procurement; stamped → it appears in /import (the import page filters `orders` to sent ones; unsent are passed as `procurementOrders` for status chips). Historical orders were backfilled with `order_date`/`created_at`. Orders created directly in /import (+ הזמנה) or by SmartUpload are stamped sent on insert; `/api/import/from-quote` auto-drafts stay NULL → land in procurement. The /import "הצעות מאושרות" sheet lists signed quotes, **plus** quotes that back a live (non-cancelled) PO but are no longer signed (signature revoked, revision awaiting re-sign) — those rows show a "ממתין לחתימה מחודשת" chip instead of the approval date, and are dropped once a signed quote claims the same order.
...
- **`components/procurement/PODocument.tsx`** — branded A4 PO document (same visual language + html2canvas/jsPDF export + measured hidden-mirror pagination as `QuoteDocument` — rows are never cut mid-page). **Language follows the currency** with a viewer override: ILS → Hebrew RTL; foreign (USD/EUR/GBP) → fully English LTR (all template labels, en-GB long date "19 July 2026", Hebrew units mapped יח׳/יחידה→pcs, מטר/מטרים→m, קומפלט/סט→set); a עברית/English toggle in both PDF modals (procurement + import) overrides via the `lang` prop. Project name follows the doc language (`projectNameHe` = projects.name for Hebrew, the PO's translated `project_name` for English; lists in /procurement always show the Hebrew projects.name). Supplier addressee, delivery date, totals in the PO currency, Fibertech-only approval block (supplier signature removed by request).
- **Documented cancellation (2026-07-27, migration `20260727_001`)**: `cancelled_at/by` + `cancel_reason` on BOTH `import_orders` and `orders` (status `'cancelled'`, no CHECK on either status column). Cancel ≠ delete: /procurement "בטל הזמנה" button (reason required) + collapsed "הזמנות שבוטלו" table; /import status picker includes בוטלה (prompts for reason, shown on the card); /production "בטל הזמנה" on the customer order (cancelled joins the collapsed completed group, never counts as overdue); pricing הזמנות tab shows the chip+reason. Billing cron (9b) skips cancelled orders. `createPO` re-checks the server for an existing order per quote right before insert — kills the race with the auto-draft from signing that once minted duplicate POs.
...
- **Anomaly check vs the approved quote**: on card open the quote's `quote_items` are loaded and aggregated by item identity `family|DN|PN|SN` — family (צינור/רוקר/מחבר/מחבר קיר/ברך/אוגן/מעבר) is keyword-derived from the product name so a pipe and a rocker sharing DN/PN/SN don't merge ("Pipe with Coupling" counts as pipe — the pipe check wins over coupling keywords, after rocker). Four anomaly categories in the orange panel (live while editing; green line when clean): quantity mismatch · approved spec missing from the PO · **extra** (in the PO but not in the quote; a blank-PN/SN row that's a completion candidate of a missing spec is not an extra) · **unit-length drift** vs the approved quote's length_m (skipped when the quote holds inconsistent lengths for a spec). Each line has a per-anomaly fix button (align qty / complete PN-SN or add row / remove, with confirm); bottom row: "רענן בדיקה" (re-fetch quote, report remaining gaps) + "תקן את כל האנומליות" (fix-all; removes any keyed extra row, keeps DN-less rows). The offending CELL is painted warning in the table (qty cell / blank PN-SN cells / description cell for extras) and clears live as gaps close. All local until שמור.
- **Attached signed pages (2026-07-24, migration `20260724_001`)**: `import_orders.attached_pages` jsonb (`[{path, page, label}]`). PO editor section "שרטוטים ונספחים חתומים" lists the deal's documents (customer-signed order scan from `order_documents`, order confirmations, quote uploads, linked quote_drawings); clicking one renders page thumbnails (pdf.js) and pages are toggled per click ("צרף את כל העמודים" too). `PODocument` self-loads `order.attached_pages` via `lib/po-attachments.ts` (one download per source, `renderAttachedPages`) and renders them as A4 annex pages ("נספח להזמנת רכש") at the end of the document + PDF export — so the customer-signed drawings ship to the supplier as an integral part of the PO.
- **/import**: `POViewButton` ("צפה בהזמנת רכש") opens the read-only PO PDF modal — on approved-quotes rows with an order and on every order card (so Nurit sees exactly what was sent); annex pages included.
...
- **Opened / last-touched dates (2026-09-24, migration `20260924_001`)**: the projects list shows two sortable columns — **נפתח** (`projects.created_at`; the 38 pre-migration projects all carry 2026-04-09, the day the data was loaded) and **עדכון** (`projects.last_updated_at`). `last_updated_at` is now maintained by DB triggers instead of scattered code paths, so it can't be forgotten: `stamp_project_row` (BEFORE UPDATE on projects) stamps `updated_at`+`last_updated_at`, and `touch_parent_project` (AFTER INSERT/UPDATE/DELETE, SECURITY DEFINER + REVOKEd from anon/authenticated) stamps the parent from 13 child tables — project_details, project_contacts, pipe_specs, project_updates, quotes, cost_inputs, attachments, orders, fitting_estimates, import_orders, customer_invoices, import_customer_deliveries, purchase_receipts. `alerts` is deliberately excluded (the cron writes there nightly and would bump every project).
...
- **Field-works section (2026-08-24)**: the standard terms' section 6 is the unified "עבודות שטח באתר הלקוח" (clauses 50–60), opening with an "אינה כוללת עבודות חבישה" clause by default. A "תוספת עבודות שטח" checkbox on draft quote cards (`toggleFieldWorksTerms` in usePricing + `lib/field-works-terms.ts:applyFieldWorks`) swaps the section to its ON build — opening "כוללת" clause + B149 execution clauses + the crew clauses — renumbers the whole document (הזמנה 61→78), and stores the result as `contract_overrides`. Checked-state detection: first clause starts with "הצעה זו כוללת" (`isFieldWorksOn`). The DB 'סטנדרטית' template was updated to the same OFF layout.
...
- **Drag-and-drop** is used in three places (cost input cards in `PricingSection.tsx`, project drawings/specs in `app/projects/[id]/page.tsx`, the import SmartUpload drop zone). All three share `lib/dropped-files.ts` (`filesFromDrop` reads `dataTransfer.items` too — iPad/Safari leaves `files` empty on a cross-app drag; `materializeFiles` reads the bytes at drop time because WebKit invalidates the blob seconds later; `safeExt` keeps storage keys ASCII). The `dragOver` state + `dragDepth` ref boilerplate is still duplicated — see Issues #12.
- **SmartUpload project hints (2026-09-23)**: the pick phase takes an optional multi-select of our project names (chips) and posts them to `/api/import/extract` as `projectHints`. The route appends a context block to the extraction prompt telling the model to use the list ONLY to recognise a project name already present in the document (return it verbatim, never copy from the list, never infer other fields). One stated project also pre-selects the system project in the review phase.
