...
- **Length-based vs piece-counted (2026-09-23)**: `isLengthBased(item_type, product_name)` in `lib/pricing.ts` — only `pipe_with_coupling`/`pipe_bare`/`roker`/`floating_roker` have a meaningful "יחידות" count (qty ÷ length_m). A piece-counted part (elbow, חיוץ, מצוף, flange…) may well carry a length, but each is one unit, so the units cell shows "—", the units input is disabled, and the fractional-units **issue gate in `updateQuoteStatus` skips it**. Applied in all 9 sites: both cost-item editors + display, the quote editor + read-only table, QuoteDocument, PODocument and the /procurement PO editor. Rows with no `item_type` (legacy, and PO lines which have no such column) fall back to the product-name keywords `defaultLengthM` uses, so nothing that worked before changes.
